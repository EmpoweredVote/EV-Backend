#!/usr/bin/env python3
"""
Census TIGER/Line Shapefile Importer

Downloads and imports geofence boundaries for Monroe County, IN and LA County, CA.
Requires: pip install geopandas psycopg2-binary requests
"""
...
SHAPEFILES = {
    ...
    "cd": f"https://www2.census.gov/geo/tiger/TIGER{YEAR}/CD/tl_{YEAR}_us_cd118.zip",
...
def main():
    ...
    try:
        ...
        print("Install with: pip install geopandas psycopg2-binary requests")
        sys.exit(1)
    ...
    try:
        ...
        cur.execute("""
            ...
            GROUP BY mtfcc
            ORDER BY mtfcc
        """, (f'census_tiger_{YEAR}',))

        print("\nImported by feature type:")
        for row in cur.fetchall():
            print(f"  {row[0] or 'NULL'}: {row[1]} features")
